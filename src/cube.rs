...
impl Cube3x3 {
    ...
    //total manhattan distance heuristic for search
    pub fn manhattan_dist(&self) -> usize {
        const EDGE_DISTS: [[u8; 12]; 12] = [
            [0, 2, 2, 2, 2, 4, 4, 4, 2, 4, 2, 4],
            [2, 0, 2, 4, 4, 2, 4, 4, 4, 4, 2, 2],
            [2, 2, 0, 2, 4, 4, 2, 4, 4, 2, 4, 2],
            [2, 2, 2, 0, 4, 4, 4, 2, 2, 2, 4, 4],
            [2, 4, 4, 4, 0, 2, 2, 2, 2, 4, 2, 4],
            [4, 2, 4, 4, 2, 0, 2, 2, 4, 4, 2, 2],
            [4, 4, 2, 4, 2, 2, 0, 2, 4, 2, 4, 2],
            [4, 4, 4, 2, 2, 2, 2, 0, 2, 2, 4, 4],
            [2, 4, 4, 2, 2, 4, 4, 2, 0, 2, 2, 4],
            [4, 4, 2, 2, 4, 4, 2, 2, 2, 0, 4, 2],
            [2, 2, 4, 4, 2, 2, 4, 4, 2, 4, 0, 2],
            [4, 2, 2, 4, 4, 2, 2, 4, 4, 2, 2, 0],
        ];
        const CORNER_DISTS: [[u8; 8]; 8] = [
            [0, 2, 4, 2, 2, 4, 6, 4],
            [2, 0, 2, 4, 4, 2, 4, 6],
            [4, 2, 0, 2, 6, 4, 2, 4],
            [2, 4, 2, 0, 4, 6, 4, 2],
        ...
        for (i, e) in self.edges.iter().enumerate() {
            total += EDGE_DISTS[i][(*e % 12) as usize] as usize;
        }
        for (i, c) in self.corners.iter().enumerate() {
            total += CORNER_DISTS[i][(*c % 8) as usize] as usize;
        }
        return total;
    }
}
